...
(define (domain CITY_MANAGER)
    ...
    (:durative-action return_UAV
        ...
        :condition (and
            ...
            (at start (free ?u))
            (at start (at ?u ?f))
            (at start (>= (*(/(power_level ?u)(power_used_rate ?u))(speed ?u)) (distance_air ?f?t)))
        )
        :effect (and
            (at start (not(at ?u ?f)))
            (at start (not(free ?u)))
